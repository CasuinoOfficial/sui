...
struct Context<'a> {
    ...
    single_signature_type_map: BTreeMap<SignatureIndex, Type>,
}
...
pub fn module(
    ...
    for func_handle in module.function_handles() {
        ...
        if runtime_id == self_id {
            ...
            for (idx, function) in cache.functions.iter().enumerate().rev() {
                ...
                if arena::to_ref(*function).name.as_ident_str() == func_name {
    ...
    let single_signature_type_map = BTreeMap::new();
    let context = Context {
        link_context,
        cache,
        module,
        function_map,
        single_signature_type_map,
    };
        ...
        .zip(module.function_defs())
    {
        if let Some(code_unit) = fun.code {
            alloc.code = code(&context, &code_unit.code)?;
        }
    }
...
pub fn code(context: &Context, bytecode: &[FF::Bytecode]) -> PartialVMResult<*const [Bytecode]> {
    context
        .cache
        .arena
        .alloc_slice(bytecode.iter().map(|bytecode| bytecode(context, bytecode)?))
}

fn bytecode(context: &Context, bytecode: &FF::Bytecode) -> PartialVMResult<Bytecode> {
    let bytecode = match bytecode {
        // Calls -- these get compiled to something more-direct here
        FF::Bytecode::Call(ndx) => Bytecode::Call(call(context, *ndx)?),

        FF::Bytecode::CallGeneric(ndx) => todo!(),
        ...
        FF::Bytecode::LdU256(n) => Bytecode::LdU256(*n),
        FF::Bytecode::LdU128(n) => Bytecode::LdU128(*n),
...
pub fn check_vector_type(
        ...
        .contains_key(signature_index)
    {
        ...
        context
            .single_signature_type_map
            .insert(*signature_index, context.cache.make_type(context.module, ty)?);
    }
    Ok(())
}
